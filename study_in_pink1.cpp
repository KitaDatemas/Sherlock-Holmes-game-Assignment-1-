...
inline bool checkIdent(const char *s, const char *se, int size){
...
inline int maxPointDiagnal(int (*matrix)[10], int row, int col){
...
inline double firstRoad(int EXP1){
    if (EXP1 >= findNearestSquareNumber(EXP1))
        return 1;
    return ((double)EXP1/findNearestSquareNumber(EXP1) + 80)/123;
}
...
double secondRoad(int &EXP1, int &HP1, int &M1, int &spending, double M1_Before, bool isOdd){
    ...
    if (M1 == 0 || spending > M1_Before && isOdd) {
        ...
        EXP1 = ceil(EXP1*1.17);
        checkEXP(EXP1);
        return (EXP1/findNearestSquareNumber(EXP1) + 80)/123;//Hết tiền return -1
    }
    //Làm hành động thứ 2
    M1 -= (EXP1 < 400)? 200 :  120;
    ...
    EXP1 = ceil(EXP1*0.9);
    checkMoney(M1);
    return (EXP1/findNearestSquareNumber(EXP1) + 80)/123;
}
...
int firstMeet(int & exp1, int & exp2, int e1) {
    ...
    if (!other){
        int D = e1 * 3 + exp1 * 7;
        if (D%2)            exp1 -= ceil(D/100);
        else                exp1 += ceil(D/200);
        checkEXP(exp1);
    }
    return exp1 + exp2;
}
...
int traceLuggage(int & HP1, int & EXP1, int & M1, int E2) {
    ...
    if (E2%2) {
        while (spending <= M1_Before) {
            int p_temp = roundTwoDigit(secondRoad(EXP1, HP1, M1, spending, M1_Before, 1));
            P2 = (EXP1 > findNearestSquareNumber(EXP1))? 1 : p_temp;
        }
    } else {int p_temp = roundTwoDigit(secondRoad(EXP1, HP1, M1, spending, M1_Before, 0));
            P2 = (EXP1 > findNearestSquareNumber(EXP1))? 1 : p_temp;
            if (M1 > 0){
                HP1 = ceil(HP1*0.83);
                EXP1 = ceil(EXP1*1.17);
...
int chaseTaxi(int & HP1, int & EXP1, int & HP2, int & EXP2, int E3) {
    ...
    maxPoint = maxPointDiagnal(taxiPoint, row, col);
    if (abs(taxiPoint[row][col]) > maxPoint)            {EXP1 = ceil(EXP1*0.88); EXP2 = ceil(EXP2*0.88); HP1 = ceil(HP1*0.9); HP2 = ceil(HP2*0.9);}
    else                                                   {EXP1 = ceil(EXP1*1.12); EXP2 = ceil(EXP2*1.12); HP1 = ceil(HP1*1.1); HP2 = ceil(HP1*1.1);}
    checkHp(HP1);
    checkHp(HP2);
    checkEXP(EXP1);
    checkEXP(EXP2);
...
int checkPassword(const char * s, const char * email) {
    bool hasSpecialChar = false, hasSe = false, dup = false;
    int count = 0, dupPos = -1, sePos = -1, warningPos = -1;
        ...
        for (int i = 0 ; i < strlen(s); i++){
            ...
            s[i] == '!' || s[i] == '@') {
                if (s[i] >= '#' && s[i] <= '%' || s[i] == '!' || s[i] == '@')
                    hasSpecialChar = true;
                if (s[i] == email[0])
                    if (checkIdent(s,email,count)) {
                        sePos = i;
                        hasSe = true;
                        break;//Nếu chứa chuỗi se trong email thì chuỗi k hợp lệ nên break
                    }
                if (i <= strlen(s) - 3 && s[i] == s[i+1] && s[i] == s[i+2]) {
                    dupPos = i;
                    dup = true;
                    break;//Nhiều hơn 2 ký tự trùng nhau
                }
                if (i == strlen(s) - 1) {
                    if (hasSpecialChar)         return -10;//Nếu ktra đến cuối hợp lệ hết thì hoàn tất
                    else                        break;//Chuỗi không có ký tự đặc biệt
                }
                continue;
            }
            else {
    ...
    if (hasSe)                                  return -(300 + sePos);
    if (dup)                                    return -(400 + dupPos);
